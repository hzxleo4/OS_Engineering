...
struct hd_request {
    int pid;            /* 请求进程的 PID */
    int io_type;        /* DEV_READ 或 DEV_WRITE */
    int block_nr;   	/* block number */
    int bytes;          /* 字节数（应为 4KB 的倍数） */
    void* buf;          /* 用户缓冲区 */
};

/* FIFO 队列（环形队列） */
#define REQ_QUEUE_SIZE  32          /* 队列容量 */
static struct hd_request request_queue[REQ_QUEUE_SIZE];
static int queue_head = 0;
static int queue_tail = 0;

static int hd_opened = 0;
